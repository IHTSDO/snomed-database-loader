...
select * from sct_refset_Language limit 10000;
...
select version() `version`,'SELECT 10000 refset_Language' `task`,current_timestamp(0),MillisecondDiff(@t2,@t1) sct,MillisecondDiff(@t3,@t2) sva,MillisecondDiff(@t4,@t3) svx,MillisecondDiff(@t5,@t4) soa,MillisecondDiff(@t6,@t5) sox,MillisecondDiff(@t6,@t1) total;