...
BEGIN
  ...
  EXECUTE 'COPY concept' || suffix || '(id, effectivetime, active, moduleid, definitionstatusid) FROM '''
        || folder || '/' || type || '/Terminology/sct2_Concept_' || type || '_' || release || '.txt'' WITH (FORMAT csv, HEADER true, ENCODING ''UTF8'', DELIMITER ''	'')';
  ...
  EXECUTE 'COPY description' || suffix || '(id, effectivetime, active, moduleid, conceptid, languagecode, typeid, term, casesignificanceid) FROM '''
        || folder || '/' || type || '/Terminology/sct2_Description_' || type || '_' || release || '.txt'' WITH (FORMAT csv, HEADER true, ENCODING ''UTF8'' , DELIMITER ''	'', QUOTE E''\b'')';

  EXECUTE 'TRUNCATE TABLE textdefinition' || suffix;
  EXECUTE 'COPY description' || suffix || '(id, effectivetime, active, moduleid, conceptid, languagecode, typeid, term, casesignificanceid) FROM '''
        || folder || '/' || type || '/Terminology/sct2_TextDefinition_' || type || '_' || release || '.txt'' WITH (FORMAT csv, HEADER true, ENCODING ''UTF8'', DELIMITER ''	'', QUOTE E''\b'')';
  ...
  EXECUTE 'COPY relationship' || suffix || '(id, effectivetime, active, moduleid, sourceid, destinationid, relationshipgroup, typeid,characteristictypeid, modifierid) FROM '''
        || folder || '/' || type || '/Terminology/sct2_Relationship_' || type || '_' || release || '.txt'' WITH (FORMAT csv, HEADER true, ENCODING ''UTF8'', DELIMITER ''	'')';
  ...
  EXECUTE 'COPY stated_relationship' || suffix || '(id, effectivetime, active, moduleid, sourceid, destinationid, relationshipgroup, typeid,  characteristictypeid, modifierid) FROM '''
        || folder || '/' || type || '/Terminology/sct2_StatedRelationship_' || type || '_' || release || '.txt'' WITH (FORMAT csv, HEADER true, ENCODING ''UTF8'', DELIMITER ''	'')';
  ...
  EXECUTE 'COPY langrefset' || suffix || '(id, effectivetime, active, moduleid, refsetid, referencedcomponentid, acceptabilityid) FROM '''
        || folder || '/' || type || '/Refset/Language/der2_cRefset_Language' || type || '-en_' || release || '.txt'' WITH (FORMAT csv, HEADER true, ENCODING ''UTF8'', DELIMITER ''	'')';
  ...
  EXECUTE 'COPY associationrefset' || suffix || '(id, effectivetime, active, moduleid, refsetid, referencedcomponentid, targetcomponentid) FROM '''
        || folder || '/' || type || '/Refset/Content/der2_cRefset_Association' || type || '_' || release || '.txt'' WITH (FORMAT csv, HEADER true, ENCODING ''UTF8'', DELIMITER ''	'')';
  ...
  EXECUTE 'COPY simplerefset' || suffix || '(id, effectivetime, active, moduleid, refsetid, referencedcomponentid) FROM '''
        || folder || '/' || type || '/Refset/Content/der2_Refset_Simple' || type || '_' || release || '.txt'' WITH (FORMAT csv, HEADER true, ENCODING ''UTF8'', DELIMITER ''	'')';
  ...
  EXECUTE 'COPY attributevaluerefset' || suffix || '(id, effectivetime, active, moduleid, refsetid, referencedcomponentid, valueid) FROM '''
        || folder || '/' || type || '/Refset/Content/der2_cRefset_AttributeValue' || type || '_' || release || '.txt'' WITH (FORMAT csv, HEADER true, ENCODING ''UTF8'', DELIMITER ''	'')';
  ...
  EXECUTE 'COPY simplemaprefset' || suffix || '(id, effectivetime, active, moduleid, refsetid,  referencedcomponentid, maptarget) FROM '''
        || folder || '/' || type || '/Refset/Map/der2_sRefset_SimpleMap' || type || '_' || release || '.txt'' WITH (FORMAT csv, HEADER true, ENCODING ''UTF8'', DELIMITER ''	'')';
  ...
  EXECUTE 'COPY extendedmaprefset' || suffix || '(id, effectivetime, active, moduleid, refsetid, referencedcomponentid, mapGroup, mapPriority, mapRule, mapAdvice, mapTarget, correlationId, mapCategoryId) FROM '''
        || folder || '/' || type || '/Refset/Map/der2_iisssccRefset_ExtendedMap' || type || '_' || release || '.txt'' WITH (FORMAT csv, HEADER true, ENCODING ''UTF8'', DELIMITER ''	'')';
END $$